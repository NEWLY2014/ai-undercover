...
这是 Agent 项目,核心原则是 **绝不写状态机替 agent 做决定**:
- 角色分配 / 计票 / 胜负 / 轮次是确定性的**游戏裁判**(纯函数引擎 `src/game/engine.ts`),不替 agent 决定"说什么 / 投谁"。
- **记忆、嫌疑分、复盘经验全部由 LLM 自己产出**,代码只负责存储与渲染,绝不解读其语义去改 agent 行为。
- 投票非法时**重问同一个 agent**,而不是替它随机投。
